...
const AddReflectionScreen = () => {
  ...
  const { currentSession, currentBAC, endSession, updateSession, sessions } = useApp();
  const [notes, setNotes] = useState('');
  const [selectedFeeling, setSelectedFeeling] = useState<'great' | 'good' | 'okay' | 'bad' | undefined>();

  // Get session from route params or current session
  ...
  const handleSave = async () => {
    if (sessionId && session) {
      ...
      try {
        await updateSession(sessionId, notes.trim() || '', selectedFeeling);
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>
          {sessionId ? 'Add Reflection' : 'End Session'}
        </Text>
        <View style={{ width: 44 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.content}>
          ...
          {sessionId ? (
            <>
              ...
              <View style={styles.summaryRow}>
                ...
                <Text style={styles.summaryValue}>{session.drinks.length}</Text>
              </View>
              <View style={styles.summaryRow}>
                <Text style={styles.summaryLabel}>Max BAC:</Text>
                <Text style={styles.summaryValue}>{session.maxBAC.toFixed(3)}%</Text>
              </View>
            </>
          ) : (
            <>
              ...
              <View style={styles.summaryRow}>
                <Text style={styles.summaryLabel}>Current BAC:</Text>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Add Reflection</Text>
        ...
        <TouchableOpacity style={styles.saveButton} onPress={handleSave}>
          <Text style={styles.saveButtonText}>
            {sessionId ? 'Save Reflection' : 'Save Session'}
          </Text>
        </TouchableOpacity>
      </ScrollView>
    </View>
